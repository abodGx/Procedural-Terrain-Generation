class MathUtil {
    ...
    static normalize(min, max, val) {
        return (val - min) / (max - min);
    }

    static ease(x) {
        x = x * x * x * x;
        return (x * (13 + x * (11 + x * (7 + x)))) / (11 + x * (7 + x * (5 + x * 22)));
    }
    ...
    static cantorPairing(a, b) {
        let sum = a + b;
        return (sum * (sum + 1) >> 1) + b;
    }

    static cantorPairingSigned(x, y) {
        return MathUtil.cantorPairing((x >= 0.0 ? 2.0 * x : (-2.0 * x) - 1.0), (y >= 0.0 ? 2.0 * y : (-2.0 * y) - 1.0));
    }

    static cantorDepairing(z) {
        const t = Math.floor((Math.sqrt(8 * z + 1) - 1) / 2),
            x = t * (t + 3) / 2 - z,
            y = z - t * (t + 1) / 2;
        return {x, y};
    }

    static triangleNormal(x1, y1, z1, x2, y2, z2, x3, y3, z3) {
        const uX = x2 - x1,
            uY = y2 - y1,
            uZ = z2 - z1,
            vX = x3 - x1,
            vY = y3 - y1,
            vZ = z3 - z1;
        return new Vector3(uY * vZ - uZ * vY, uZ * vX - uX * vZ, uX * vY - uY * vX).normalize();
    }
...
class Vector3 {
    ...
    equal(other) {
        return other.x === this.x && other.y === this.y && other.y === this.y;
    }
...
class Transform {
    ...
    static negate(direction) {
        const result = [];
        for (const pos of direction) result.push(-pos);
        return result;
    }
