// noinspection JSUnusedGlobalSymbols
class Camera extends Transformable {
    constructor(gl,
                width,
                height,
                fov = 45,
                near = 0.1,
                far = 10 * Terrain.worldSize * Terrain.chunkSize) {
        super();
        //Set up the perspective matrix
        this.projectionMatrix = new Float32Array(16);
        Matrix4.perspective(this.projectionMatrix, fov, width / height, near, far);

        this.viewMatrix = new Float32Array(16);	//Cache the matrix that will hold the inverse of the transform.
        this.mode = Camera.MODE_FREE;			        //Set what sort of control mode to use.
    }

    panX(v) {
        if (this.mode === Camera.MODE_ORBIT) return; // Panning on the X Axis is only allowed when in free mode
        this.updateViewMatrix();
        this.transform.position.x += this.transform.right[0] * v;
        this.transform.position.y += this.transform.right[1] * v;
        this.transform.position.z += this.transform.right[2] * v;
    }

    panY(v) {
        this.updateViewMatrix();
        this.transform.position.y += this.transform.up[1] * v;
        if (this.mode === Camera.MODE_ORBIT) return; //Can only move up and down the y axix in orbit mode
        this.transform.position.x += this.transform.up[0] * v;
        this.transform.position.z += this.transform.up[2] * v;
    }

    panZ(v) {
        this.updateViewMatrix();
        if (this.mode === Camera.MODE_ORBIT) {
            this.transform.position.z += v; //orbit mode does translate after rotate, so only need to set Z, the rotate will handle the rest.
        } else {
            //in free-mode to move forward, we need to move based on our forward, which is relative to our current rotation
            this.transform.position.x += this.transform.forward[0] * v;
            this.transform.position.y += this.transform.forward[1] * v;
            this.transform.position.z += this.transform.forward[2] * v;
        }
    }

    updateViewMatrix() {
        //Optimize camera transform update, no need for scale
        if (this.mode === Camera.MODE_FREE) {
            this.transform.matView.reset()
                .vtranslate(this.transform.position)
                .rotateY(this.transform.rotation.y * Transform.deg2Rad)
                .rotateX(this.transform.rotation.x * Transform.deg2Rad)
                .rotateZ(this.transform.rotation.z * Transform.deg2Rad);

        } else {
            this.transform.matView.reset()
                .rotateY(this.transform.rotation.y * Transform.deg2Rad)
                .rotateX(this.transform.rotation.x * Transform.deg2Rad)
                .rotateZ(this.transform.rotation.z * Transform.deg2Rad)
                .vtranslate(this.transform.position);
        }

        this.transform.updateDirection();
        Matrix4.invert(this.viewMatrix, this.transform.matView.raw);
        return this.viewMatrix;
    }

    getTranslatelessMatrix() {
        const mat = new Float32Array(this.viewMatrix);
        mat[12] = mat[13] = mat[14] = 0.0; //Reset Translation position in the Matrix to zero.
        return mat;
    }
}

Camera.MODE_FREE = 0;	//Allows free movement of position and rotation, basicly first person type of camera
Camera.MODE_ORBIT = 1;	//Movement is locked to rotate around the origin, Great for 3d editors or a single entity viewer

class CameraController {
    static rotationRate = 170;
    static panRate = 600;
    static zoomRate = 50;
    delegatedTarget;
    camera;
    box;

    constructor(gl, delegatedTarget, boundingBox, camera) {
        const oThis = this;
        this.delegatedTarget = delegatedTarget;
        this.box = boundingBox;
        this.camera = camera;

        this.offsetX = this.box.left;						//Help calc global x,y mouse cords.
        this.offsetY = this.box.top;

        this.prevX = 0;									//Previous X,Y position on mouse move
        this.prevY = 0;

        this.onUpHandler = e => oThis.onMouseUp(e);		//Cache func reference that gets bound and unbound a lot
        this.onMoveHandler = e => oThis.onMouseMove(e)

        this.delegatedTarget.addEventListener("mousedown", e => oThis.onMouseDown(e));		//Initializes the up and move events
        this.delegatedTarget.addEventListener("mousewheel", e => oThis.onMouseWheel(e));	//Handles zoom/forward movement
    }

    //Transform mouse x,y cords to something usable by the canvas.
    getMouseVec2(e) {
        return {x: e.pageX - this.offsetX, y: e.pageY - this.offsetY};
    }

    //Begin listening for dragging movement
    onMouseDown(e) {
        this.prevX = e.pageX - this.offsetX;
        this.prevY = e.pageY - this.offsetY;

        this.delegatedTarget.addEventListener("mouseup", this.onUpHandler);
        this.delegatedTarget.addEventListener("mousemove", this.onMoveHandler);

    }

    //End listening for dragging movement
    onMouseUp() {
        this.delegatedTarget.removeEventListener("mouseup", this.onUpHandler);
        this.delegatedTarget.removeEventListener("mousemove", this.onMoveHandler);
    }

    onMouseWheel(e) {
        const delta = -Math.max(-1, Math.min(1, (e.wheelDelta || -e.detail))); //Try to map wheel movement to a number between -1 and 1
        this.camera.panZ(delta * ((1000 * CameraController.zoomRate) / this.box.height));		//Keep the movement speed the same, no matter the height diff
        this.camera.fireEvent(this.camera.transform);
    }

    onMouseMove(e) {
        const x = e.pageX - this.offsetX,	//Get X,y where the canvas's position is origin.
            y = e.pageY - this.offsetY,
            dx = x - this.prevX,		//Difference since last mouse move
            dy = y - this.prevY;
        if (e.shiftKey) {
            this.camera.panX(-dx * (CameraController.panRate / this.box.width));
            this.camera.panY(dy * (CameraController.panRate / this.box.height));
        } else {
            this.camera.transform.rotation.y += dx * (-CameraController.rotationRate / this.box.width);
            this.camera.transform.rotation.x += dy * (-CameraController.rotationRate / this.box.height);
        }

        this.prevX = x;
        this.prevY = y;
        this.camera.fireEvent(this.camera.transform);

    }
}